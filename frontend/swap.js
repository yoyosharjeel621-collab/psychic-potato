async function swapTokens() {
  ...
  try {
    ...
    document.getElementById("swapUI").innerHTML = `
      ...
      <button onclick="executeSwap('${fromToken}','${toToken}','${amountWei}')">
      Execute Swap
      </button>
    `;
...
async function executeSwap(fromToken,toToken,amountWei){
  ...
  try {
    ...
    const txUrl =
    `https://api.1inch.io/v5.0/1/swap?fromTokenAddress=${fromToken}&toTokenAddress=${toToken}&amount=${amountWei}&fromAddress=${await signer.getAddress()}&slippage=1`;
    ...
    const tx = await signer.sendTransaction({
      ...
      value: txData.tx.value
        ? ethers.BigNumber.from(txData.tx.value)
        : undefined,
      gasLimit: txData.tx.gas
        ? ethers.BigNumber.from(txData.tx.gas)
        : undefined
    });

    alert("Swap Success! Tx: " + tx.hash);
